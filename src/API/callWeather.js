...
export const callWeather = async (mes) => { //@ 取得"天氣"
    ...
    let rule = /台北|臺北|基隆|新北|宜蘭|新竹市|新竹縣|桃園|苗栗|台中市|臺中市|台中|臺中|彰化|南投|嘉義市|嘉義縣|雲林|台南|臺南|台南|臺南|高雄|屏東|台東|花蓮|澎湖|金門|連江/ //設定正規表達 rule
    ...
    if (fuzzyQuery(rule, newMes)[0]) { //模糊查詢 function ，判斷如果 rule 為 true 關鍵字配對
        try {
            ...
            const data = response.data; // GET response
